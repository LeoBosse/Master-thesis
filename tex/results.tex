...
%  \end{minipage}
  \begin{minipage}[h]{0.45\textwidth}
	...
	\includegraphics[width=1.\linewidth]{pic/results/Weniger_SUM_b0-2_E4,7-0,31GeV.png}
  	\subcaption{Some weniger plots to show the GC excess. TODO{don't show the model, only the data}}
  	\label{fig:weniger_plot}
  \end{minipage}
  \hfill
  \begin{minipage}[h]{0.45\textwidth}
...
In addition to the basic templates, a fourth component is needed to fit the data in the disk and the bubbles. This characteristic high energy hardening of the spectrum was thought to be induced by non propagated CR, CR that do not prpagate through dense gas clouds or highly convoluted magnetic fields. For example in the bubbles, where source eject particles without passing through dust clouds, or simply near sources, where CR do not have time to collide yet.
